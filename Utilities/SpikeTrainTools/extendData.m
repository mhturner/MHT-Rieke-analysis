function x = extendData(type,mode,x,lf,location)
%EXTENDDATA Extend a Vector or a Matrix.
%
%   Y = EXTENDDATA(TYPE,MODE,X,L,LOC) or
%   Y = EXTENDDATA(TYPE,MODE,X,L)
%
...
%        If the signal length is odd, EXTENDDATA adds an extra-sample
...
%   $Revision: 1.10.4.7 $  $Date: 2012/07/05 17:26:40 $
...
switch type
  ...
  case {2,'2','2d'}
      ...
      if ~ismatrix(x)
          y = cell(1,3);
          for k=1:3
              y{k} = extendData('2D',mode,x(:,:,k),lf,[locRow locCol]);
          end
          x = cat(3,y{:});
          return
      end
   ...
   case {'ar','ac','addrow','addcol'}
      ...
      if ismatrix(x)
          x = extendData('2D',mode,x,lf,location);
      else
          y = cell(1,3);
          x = double(x);
          for k=1:3
              y{k} = extendData('2D',mode,x(:,:,k),lf,location);
          end
          x = cat(3,y{:});
      end
end
